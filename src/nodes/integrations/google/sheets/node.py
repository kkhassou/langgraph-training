...
class SheetsNode(BaseNode):
    """Node for Google Sheets operations via MCP (Model Context Protocol) server"""
...
sheets_node = SheetsNode()


async def sheets_node_handler(input_data: SheetsInput) -> Dict[str, Any]:
    """Handler function for Google Sheets MCP node endpoint"""
    result = await sheets_node.execute(input_data.model_dump())
