...
class DocsNode(BaseNode):
    """Node for Google Docs operations via MCP (Model Context Protocol) server"""
...
async def docs_node_handler(input_data: DocsInput) -> Dict[str, Any]:
    """Handler function for Google Docs MCP node endpoint"""
    result = await docs_node.execute(input_data.model_dump())
