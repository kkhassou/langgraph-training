"""Google Apps Script Integration Node

Google Apps Scriptとの統合を行うノード実装。
...
class AppsScriptInput(BaseModel):
...
class AppsScriptNode(BaseNode):
    """Node for Google Apps Script operations via MCP (Model Context Protocol) server"""
...
apps_script_node = AppsScriptNode()


async def apps_script_node_handler(input_data: AppsScriptInput) -> Dict[str, Any]:
    """Handler function for Google Apps Script MCP node endpoint"""
    result = await apps_script_node.execute(input_data.model_dump())
