...
from src.mcp.google.calendar.client import CalendarMCPService
...
class CalendarNode(BaseNode):
    """Node for Google Calendar operations via MCP (Model Context Protocol) server"""
...
async def calendar_node_handler(input_data: CalendarInput) -> Dict[str, Any]:
    """Handler function for Google Calendar MCP node endpoint"""
    result = await calendar_node.execute(input_data.model_dump())
