...
class SlidesNode(BaseNode):
    """Node for Google Slides operations via MCP (Model Context Protocol) server"""
...
slides_node = SlidesNode()


async def slides_node_handler(input_data: SlidesInput) -> Dict[str, Any]:
    """Handler function for Google Slides MCP node endpoint"""
    result = await slides_node.execute(input_data.model_dump())
