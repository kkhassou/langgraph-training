"""Google Forms Integration Node

Google Formsとの統合を行うノード実装。
...
class FormsNode(BaseNode):
    """Node for Google Forms operations via MCP (Model Context Protocol) server"""
...
forms_node = FormsNode()


async def forms_node_handler(input_data: FormsInput) -> Dict[str, Any]:
    """Handler function for Google Forms MCP node endpoint"""
    result = await forms_node.execute(input_data.model_dump())
