...
class SlackNode(BaseNode):
    ...
    async def execute(self, state: NodeState) -> NodeState:
        ...
        try:
            ...
            action = state.data.get("action", SlackActionType.GET_MESSAGES)
...
async def slack_node_handler(input_data: SlackInput) -> SlackOutput:
    """Standalone handler for Slack node API endpoint"""
    try:
        node = SlackNode()
        state = NodeState()
        state.data.update({
